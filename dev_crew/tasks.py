...
def combat_ui_task(developer: Agent) -> Task:
    ...
    return Task(
        description=(
            "아래 순서대로 도구를 한 번에 하나씩 호출하라. 다른 파일은 건드리지 마라.\n\n"
            "1. read_project_file 호출: file_path='ui/terminal_ui.py'\n\n"
            "2. patch_project_file 호출 — show_combat_preview 추가:\n"
            "   file_path='ui/terminal_ui.py'\n"
            "   old_code='        return result[0]'\n"
            "   new_code=아래 코드 (들여쓰기 4칸 = 클래스 메서드 레벨):\n"
            "   ```\n"
            "        return result[0]\n\n"
            ...
            "   ```\n"
            "   주의: old_code는 반드시 '        return result[0]' (공백 8개)이어야 한다.\n"
            "   class TerminalUI: 줄은 건드리지 마라.\n\n"
            "3. patch_project_file 호출 — show_faction_change_animation 추가:\n"
            "   file_path='ui/terminal_ui.py'\n"
            "   old_code='        input(\\'Press Enter to start battle...\\')'\n"
            "   new_code=아래 코드:\n"
            "   ```\n"
            "        input('Press Enter to start battle...')\n\n"
            ...
            "   ```\n\n"
            "4. python_runner 호출:\n"
            "   code='import sys, ast; sys.path.insert(0,\".\")\n"
            "   ast.parse(open(\"ui/terminal_ui.py\").read())\n"
            "   from ui.terminal_ui import TerminalUI; ui = TerminalUI()\n"
            "   assert hasattr(ui, \"show_combat_preview\"), \"FAIL show_combat_preview\"\n"
            "   assert hasattr(ui, \"show_faction_change_animation\"), \"FAIL show_faction_change_animation\"\n"
            "   print(\"STEP1_OK\")'\n\n"
            "규칙: 기존 코드 삭제 금지. self.console 사용. 타입 힌트 필수."
        ),
...
def combat_manager_live_task(developer: Agent) -> Task:
    """Step 2: combat_manager.py — with Live 제거, console.clear 삽입."""
    return Task(
        description=(
            "아래 순서대로 도구를 호출하라. terminal_ui.py와 turn_manager.py는 건드리지 마라.\n\n"
            "1. read_project_file 호출: file_path='game/combat_manager.py'\n"
            "2. combat_manager.py 수정 작업 (최소 변경 원칙):\n"
            "   A) 만약 'with Live(auto_refresh=False) as live:' 블록이 존재하면:\n"
            "      - 해당 with 블록을 제거하고 내부 코드만 같은 들여쓰기로 남겨라 (1개 patch).\n"
            "      - 즉, 'with Live(auto_refresh=False) as live:\\n\\n                console.print(...)' 를\n"
            "        'console.print(...)' 로 교체한다.\n"
            "   B) siege_battle 메서드 시작의 console.rule(...) 바로 앞에 console.clear() 삽입 (1개 patch).\n"
            "      - old_code 예시 (공성전 선언 부분, 정확한 앵커로 사용):\n"
            "        'console.print()\\n        console.rule(f\"[bold red]⚔️  공성전 개시:'\n"
            "      - new_code:\n"
            "        'console.clear()\\n        console.print()\\n        console.rule(f\"[bold red]⚔️  공성전 개시:'\n"
            "   C) 'time.sleep(0.3)' 를 'time.sleep(1.2)' 로 교체 (1개 patch) — 전투를 더 느리게.\n"
            "3. from rich.live import Live 는 그대로 두어라 (제거 금지).\n"
            "4. python_runner 호출:\n"
            "code='import sys; sys.path.insert(0,\".\")\n"
            "from game.combat_manager import CombatManager\n"
            "src = open(\"game/combat_manager.py\").read()\n"
            "assert \"Live\" in src, \"FAIL: Live import removed\"\n"
            "assert \"console.clear\" in src, \"FAIL: console.clear not in combat_manager\"\n"
            "print(\"STEP2_OK\")'\n\n"
            "코드 규칙: from __future__ import annotations, 기존 코드 삭제 금지.\n"
            "⚠️ with Live(...) 블록이 없으면 A단계 건너뛰어도 됨."
        ),
...
def combat_connect_task(developer: Agent) -> Task:
    ...
    return Task(
        description=(
            ...
            "2. Patch A — siege_battle 호출 전에 show_combat_preview 삽입:\n"
            "   patch_project_file 호출:\n"
            "     file_path='game/turn_manager.py'\n"
            "     old_code: '        result = self.combat_manager.siege_battle(attacker, defender, town, attacker_general=hero)'\n"
            "     new_code:\n"
            ...
            "3. Patch B — 승리 분기에 faction 변경 + show_faction_change_animation 삽입:\n"
            "   patch_project_file 호출:\n"
            "     file_path='game/turn_manager.py'\n"
            "     old_code (공백 정확히 유지, 들여쓰기 8/12칸):\n"
            ...
            "print(\"STEP3_OK\")'\n\n"
            "코드 규칙: from __future__ import annotations, 기존 코드 삭제 금지.\n"
            "⚠️ old_code 의 들여쓰기(공백 8개)를 정확히 맞춰야 patch가 성공한다."
        ),
...
def review_feature_task(feature_request: str, reviewer: Agent) -> Task:
    ...
    return Task(
        description=(
            ...
            "   - combat_manager.py 에 'console.clear' 문자열 — 있음/없음\n"
