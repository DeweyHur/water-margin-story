...
def combat_ui_task(developer: Agent) -> Task:
    ...
    return Task(
        description=(
            ...
            "1. read_project_file 호출: file_path='ui/terminal_ui.py'\n\n"
            "2. show_combat_preview 메서드 추가:\n"
            "   patch_project_file 호출:\n"
            "     file_path='ui/terminal_ui.py'\n"
            "     old_code: 정확히 파일의 마지막 줄 '        return result[0]'\n"
            "     new_code:\n"
            "       '        return result[0]\\n\\n"
            ...
            "        self.console.clear()\\n"
            "        self.console.print(Panel(f\"[bold red]⚔️  공성 개시[/]  {h_name} ===► {t_name}\", style=\"bold\"))\\n"
            "        self._render_static_map(state, hero_town_id, highlight_town_id=target_town_id)\\n"
            "        self.console.print(f\"  [bold]{h_name}[/] ===► [bold red]{t_name}[/]  공성 개시!\")\\n"
            "        input(\"전투 시작 — Enter 키를 누르세요...\")'\n\n"
            "   ⚠️ old_code 는 반드시 '        return result[0]' (공백 8개 + return) 이어야 한다.\n"
            "   ⚠️ class TerminalUI: 줄은 절대 건드리지 마라.\n\n"
            "3. show_faction_change_animation 메서드 추가:\n"
            "   patch_project_file 호출:\n"
            "     file_path='ui/terminal_ui.py'\n"
            "     old_code: '        input(\"전투 시작 — Enter 키를 누르세요...\")'\n"
            "     new_code:\n"
            "       '        input(\"전투 시작 — Enter 키를 누르세요...\")\\n\\n"
            ...
            "            time.sleep(0.5)\\n"
            "        self.console.clear()\\n"
            "        self.console.print(Panel(f\"[bold green]🏳️  {t_name} 점령 완료!  {old_faction_id} → {new_faction_id}[/]\", style=\"green\"))'\n\n"
            "4. python_runner 호출:\n"
            "code='import sys; sys.path.insert(0,\".\")\n"
            "import ast\n"
            "src = open(\"ui/terminal_ui.py\").read()\n"
            "ast.parse(src)\n"
            "from ui.terminal_ui import TerminalUI\n"
            "ui = TerminalUI()\n"
            ...
            "⚠️ 가장 중요: class TerminalUI: 정의를 절대 수정하지 마라. 메서드는 클래스 안에 추가해야 한다."
        ),
